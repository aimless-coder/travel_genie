    ...
    >
      ...
      {loading ? (
        ...
        <div className="my-2 flex flex-col gap-2 w-full">
          ...
          <div className="flex items-center gap-3">
            <FaMoneyBillWave />
            <h2 className="text-xs">{Math.floor(hotel?.property?.priceBreakdown?.grossPrice?.value)} <span className="font-medium text-gray-500">{hotel?.property?.priceBreakdown?.grossPrice?.currency}</span> </h2>
          </div>
          <div className="flex items-center gap-3">
          <FaStar />
